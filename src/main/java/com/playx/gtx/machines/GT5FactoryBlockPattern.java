...
/// A class to assist porting shapes of multis from GT5U
public class GT5FactoryBlockPattern {
    ...
    private GT5FactoryBlockPattern(RelativeDirection charDir, RelativeDirection stringDir, RelativeDirection aisleDir) {
        depth = new ArrayList<>();
     ...
     */
    public GT5FactoryBlockPattern aisleRepeatable(int minRepeat, int maxRepeat, String... aisle) {
        ArrayUtils.reverse(aisle);
     ...
     */
    public GT5FactoryBlockPattern aisle(String... aisle) {
     ...
     */
    public GT5FactoryBlockPattern setRepeatable(int minRepeat, int maxRepeat) {
        if (minRepeat > maxRepeat)
            throw new IllegalArgumentException("Lower bound of repeat counting must smaller than upper bound!");
     ...
     * Set last aisle repeatable
     */
    public GT5FactoryBlockPattern setRepeatable(int repeatCount) {
        return setRepeatable(repeatCount, repeatCount);
    }

    public static GT5FactoryBlockPattern start() {
        return new GT5FactoryBlockPattern(RelativeDirection.LEFT, RelativeDirection.UP, RelativeDirection.FRONT);
    }

    public static GT5FactoryBlockPattern start(RelativeDirection charDir, RelativeDirection stringDir, RelativeDirection aisleDir) {
        return new GT5FactoryBlockPattern(charDir, stringDir, aisleDir);
    }

    public GT5FactoryBlockPattern where(String symbol, TraceabilityPredicate blockMatcher) {
        return this.where(symbol.charAt(0), blockMatcher);
    }

    public GT5FactoryBlockPattern where(char symbol, TraceabilityPredicate blockMatcher) {
